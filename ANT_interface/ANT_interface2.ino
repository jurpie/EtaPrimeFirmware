...
void readPowerMeter(uint8_t *pwrRx, uint8_t print, uint16_t *time_interval, float* power) {
...
void simulate(float power, uint16_t time_interval) {
...
void loop() {
	...
	if (Serial1.available()){
		if ((m = receiveANT(rxBuffer)) > 0){
			if (m == 9) {
				readPowerMeter(rxBuffer,0 /*don't print*/, &time_int, &power);
			} 
        }
	}
	simulate(power, time_int);
